...
class InterpolatedRootDistance_distancer(DistancerInterface):
    ...
    def _foo(self,i, movelet, ndarray_pivot, spatioTemporalColumns):
        ...
        for j, trajectory in enumerate( tqdm(ndarray_pivot, disable=True, position=i+1, leave=True)):
            best_i, best_score = InterpolatedRootDistanceBestFitting(trajectory=trajectory, movelet=movelet,
...
def InterpolatedRootDistanceBestFitting(trajectory, movelet, spatioTemporalColumns, window=None, normalizer=NormalizerInterface()):  # nan == end
